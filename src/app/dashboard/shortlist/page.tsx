...
export default function ShortlistPage() {
  ...
  const { data, isLoading: developersLoading } = useDevelopers(
    shortlistIds.length === 0
      ? { page: 1, limit: ITEMS_PER_PAGE, ids: [] }
      : { ids: shortlistIds, page: currentPage, limit: ITEMS_PER_PAGE },
  );

  const developers = shortlistIds.length === 0 ? [] : data?.results ?? [];
  const totalPages = Math.ceil(shortlistCount / ITEMS_PER_PAGE);

  const handleRemove = async (developerId: string) => {
    const success = await removeFromShortlist(developerId);
    if (success) {
